...
const tabsBtns = document.querySelectorAll("[data-tab]");
const tabsProducts = Array.from(document.querySelectorAll(".card[data-value]"));

tabsBtns.forEach((btn) => {
  btn.addEventListener("click", function () {
    let currentDataAttr = btn.dataset.tab;

    tabsProducts.forEach((item) => {
      if (item.dataset.value !== currentDataAttr) {
        item.classList.add("none");
      } else {
        item.classList.remove("none");
      }
      if (currentDataAttr === "all") {
        item.classList.remove("none");
      }

      Array.from(tabsBtns).find((item) => {
        if (item.dataset.tab === currentDataAttr) {
